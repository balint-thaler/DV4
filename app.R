  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "plotly",
              h1('Political rights data for selected region, countries and year - lower = better'),
...
server <- function(input, output, session) {
  ...
  output$progressBox_free <- renderValueBox({filtered_data <- data[data$Status == "F" & data$Region_Name == input$region & data$year == input$my_year & data$country %in% input$my_country, ]
  ...
  output$progressBox_partfree <- renderValueBox({filtered_data2 <- data[data$Status == "PF" & data$Region_Name == input$region & data$year == input$my_year & data$country %in% input$my_country, ]
  ...
  output$progressBox_nonfree <- renderValueBox({filtered_data3 <- data[data$Status == "NF" & data$Region_Name == input$region & data$year == input$my_year & data$country %in% input$my_country, ]
  ...
  my_reactive_df <- reactive({
    get_filtered_data(region_name = input$region, year = input$my_year, country = input$my_country)
  }) 
