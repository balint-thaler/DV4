...
get_filtered_data <- function(region_name, year, country) {
  full_data <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2022/2022-02-22/freedom.csv')
  full_data <- data.frame(full_data)
  filtered_data <- full_data[full_data$Region_Name == toString(region_name) & full_data$year == year & full_data$country %in% country, ]
